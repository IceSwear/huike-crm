...
public class TbClueServiceImpl implements ITbClueService {
	...
	public ImportResultDTO importCluesData(TbClueExcelVo data) {
		...
		TbClue tbClue = new TbClue();
		BeanUtils.copyProperties(data,tbClue);
		...
		String dictValue = sysDictDataMapper.selectDictValue("course_subject", subject);
		tbClue.setSubject(dictValue);

		String channel = data.getChannel();
		String dictValue2 = sysDictDataMapper.selectDictValue("clues_item",channel);
		tbClue.setChannel(dictValue2);

		String level = data.getLevel();
		String dictValue3= sysDictDataMapper.selectDictValue("clues_level",level);
		tbClue.setLevel(dictValue2);

		String sex = data.getSex();
		String dictValue4= sysDictDataMapper.selectDictValue("sys_user_sex",sex );
		tbClue.setLevel(dictValue4);
		/**
		 * 设置数据状态为待跟进
		 * clue.setStatus(TbClue.StatusType.UNFOLLOWED.getValue());
		 */
