...
public class RuleStrategy implements Rule {
    ...
    public Boolean loadRule(TbClue clue) {
        ...
        if (clue.getSubject().equals(subjectJAVA.getDictValue())) {
            //1.将想要学java的分配给zhangsan
            int distribute = distribute(clue, zhangsan);
            if (distribute > 0) {
                return true;
            }
        } else if (clue.getSubject().equals(subjectHtml.getDictValue())) {
            //2.将想要学前端的分配给zhangsan1   clue,zhangsan1
            int distribute = distribute(clue, zhangsan1);
            if (distribute > 0) {
                return true;
            }
        } else {
    ...
    private int distribute(TbClue clue, SysUser user) {
