...
import com.huike.contract.domain.vo.PieChartVO;
...
//线索折线图
    ...
    //饼图
    @Override
    public List<PieChartVO> subjectStatisticst(String beginCreateTime, String endCreateTime) {
        List<PieChartVO> list= contractMapper.getsubnum(beginCreateTime,endCreateTime);
        for (PieChartVO pieChartVO : list) {
            String subject = pieChartVO.getSubject();
            String course_subject = sysDictDataMapper.selectDictLabel("course_subject", subject);
            pieChartVO.setSubject(course_subject);
        }
        return  list;
    }
    @Override
    ...
    public VulnerabilityMapVo getVulnerabilityMap(String beginCreateTime, String endCreateTime) {
        VulnerabilityMapVo vo=reportMpper.getVulnerabilityMap( beginCreateTime, endCreateTime);
        return vo;
    }
    /**
     * XIANSUO
     */
    @Override
    public List<salesStatisticvo> salesStatistic(String beginCreateTime, String endCreateTime) {
       return reportMpper.salesStatistic( beginCreateTime, endCreateTime);
    }
    /**
     * 商机龙虎
     */
    ...
    public List<Map<String, Object>> businessChangeStatistics(String beginCreateTime, String endCreateTime) {
        List<Map<String, Object>> mapList = reportMpper.businessChangeStatistics(beginCreateTime, endCreateTime);
        return mapList;
    }
    ...
    public IndexTodayInfoVO getTodayInfo() {
        ...
        try {
            //3 封装结果集对象
            result.setTodayCluesNum(reportMpper.getCluesNum2(username));
            result.setTodayBusinessNum(reportMpper.getBusinessNum2(username));
            result.setTodayContractNum(reportMpper.getContractNum2(username));
            result.setTodaySalesAmount(reportMpper.getSalesAmount2(username));
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    @Override
    public IndexVo getIndexVo(String beginCreateTime, String endCreateTime) {
